...
class SparseLinear(nn.Module):
    ...
    def __init__(self, layer, metric_type='wanda', wise_dim='row') -> None:
        ...
        if (metric_type == 'wanda' or metric_type == 'pqp' or self.metric_type == 'ria' or self.metric_type == 'bawa' or self.metric_type == 'pqp-metric'
                or self.metric_type == 'qw_pqp' or self.metric_type == 'qw_wanda_pqp' or self.metric_type == 'qw_bawa_pqp' or self.metric_type == 'wanda_pqp' or self.metric_type == 'bawa_pqp'):
    ...
    # def get_quantization_error_var(self):
    #     self.update_sparsity()
    #     prune_mask = self.get_prune_mask()
    #     quantization_error = torch.mul(self.quantization_error, prune_mask)
    #     var = torch.var(quantization_error, unbiased=False)
    #     return var

    # def get_quantization_error(self, input: torch.Tensor):
    #     quantization_error = self.quantization_error.detach()
    #     self.update_sparsity()
    #     prune_mask = self.get_prune_mask()
    #     quantization_error = torch.mul(quantization_error, prune_mask)
    #     print('input.shape:', input.shape)
    #     print('quantization_error_shape:', quantization_error.shape)
    #     out = self.linear_func(input, quantization_error, self.bias)
    #     return out
...
# if self.use_error:
        ...
        #     # out = self.linear_func(input, quantization_error, self.bias)
        # else:
# # 计算 M_l^T * 1，其中 1 是全1的列向量
# M_l_transpose_1 = prune_mask.T @ torch.ones(prune_mask.shape[0], device=self.device)  # 等价于 M_l.T @ torch.ones(M_l.shape[0])
# # 创建对角矩阵 diag(M_l^T * 1)
# diag_matrix = torch.diag(M_l_transpose_1)
# diag_matrix = 1e-7 * diag_matrix
# # print(diag_matrix)
# # 计算 X_l^T @ diag_matrix @ X_l
# input = input.squeeze(dim=0)
# # print(input)
# result_matrix = input @ diag_matrix @ input.T
# # print(result_matrix)
# # 计算矩阵的迹（trace）
# self.trace_value = torch.trace(result_matrix)
# # print(self.trace_value)
