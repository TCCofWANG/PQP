    ...
    for i in range(len(layers)):
        ...
        if args.wbits < 16:
            ...
            if args.quant_type == 'rtn':
                ...
                for name in sparse_layers:
                    ...
                    # m.cuda()
                    sparse_layers[name].weight.data = pseudo_quantize_tensor(
                        sparse_layers[name].weight.data, n_bit=args.wbits, **q_config
                    )
                    # m.cpu()
        ...
        if args.metric_type == 'wanda':
            ...
            for name in sparse_layers:
                ...
                # W = Init_W[indexed_name]
                # W_bawa = (compute_importance(W) * ((torch.sqrt(X)) ** 0.5))
                W_metric = torch.abs(sparse_layers[name].weight.data) * mms(W_ria)
                sparse_layers[name].W_metric = W_metric
        elif args.metric_type == 'qw_pqp':
            for name in sparse_layers:
                X = sparse_layers[name].scaler_row.reshape((1, -1))
                W = sparse_layers[name].weight.data
                W_ria = ((torch.abs(W) / torch.sum(torch.abs(W), dim=0) + torch.abs(W) / torch.sum(torch.abs(W),dim=1).reshape(-1,1)) * (torch.sqrt(X)) ** 0.5)
                W_metric = torch.abs(sparse_layers[name].weight.data) * mms(W_ria)
                sparse_layers[name].W_metric = W_metric
        elif args.metric_type == 'qw_wanda_pqp':
            for name in sparse_layers:
                X = sparse_layers[name].scaler_row.reshape((1, -1))
                W = sparse_layers[name].weight.data
                W_wanda = torch.abs(W) * torch.sqrt(X)
                W_metric = torch.abs(sparse_layers[name].weight.data) * mms(W_wanda)
                sparse_layers[name].W_metric = W_metric
        elif args.metric_type == 'qw_bawa_pqp':
            for name in sparse_layers:
                X = sparse_layers[name].scaler_row.reshape((1, -1))
                W = sparse_layers[name].weight.data
                W_bawa = (compute_importance(W) * ((torch.sqrt(X)) ** 0.5))
                W_metric = torch.abs(sparse_layers[name].weight.data) * mms(W_bawa)
                sparse_layers[name].W_metric = W_metric
        elif args.metric_type == 'wanda_pqp':
            for name in sparse_layers:
                indexed_name = f'{name}_layer_{i}'
                X = sparse_layers[name].scaler_row.reshape((1, -1))
                W = Init_W[indexed_name]
                ...
                W_metric = torch.abs(sparse_layers[name].weight.data) * mms(W_wanda)
                sparse_layers[name].W_metric = W_metric
        elif args.metric_type == 'bawa_pqp':
            for name in sparse_layers:
                indexed_name = f'{name}_layer_{i}'
                X = sparse_layers[name].scaler_row.reshape((1, -1))
                W = Init_W[indexed_name]
            ...
            for name in sparse_layers:
                ...
                X = sparse_layers[name].scaler_row.reshape((1, -1))
                W_metric = (
                        compute_importance(sparse_layers[name].weight.data) *
                        ((torch.sqrt(X)) ** 0.5)
                )
                W_mask = (torch.zeros_like(W_metric) == 1)  ## initialize a mask to be all False
                sort_res = torch.sort(W_metric, dim=-1, stable=True)
...
def compute_importance(W, theta1=1.0, theta2=1.0):
    row_norm = W.norm(p=2, dim=-1).unsqueeze(1)  # shape: (C_out, 1)
    col_norm = W.norm(p=2, dim=0).unsqueeze(0)   # shape: (1, C_in)

    row_metric = torch.abs(W) * (1.0 / (row_norm ** theta1))  # (C_out, C_in)
    col_metric = (1.0 / (col_norm ** theta2)) * torch.abs(W)  # (C_out, C_in)
    ...
    for i in range(len(layers)):
        ...
        for name in subset:
            ...
            if args.metric_type == 'wanda':
                ...
                PMW = (compute_importance(W) * ((torch.sqrt(X)) ** 0.5))
            elif args.metric_type == 'pruner-zero':
                G = gradients[indexed_name]
