...
def load_quant(model, checkpoint, wbits, groupsize=-1, fused_mlp=True, eval=True, warmup_autotune=True):
    ...
    for name in ['lm_head']:
        if name in layers:
            del layers[name]
    quantize.make_quant_linear(model, layers, wbits, groupsize)
    ...
    if eval:
        quantize.make_quant_attn(model)
        quantize.make_quant_norm(model)
        if fused_mlp:
            quantize.make_fused_mlp(model)

    if warmup_autotune:
        quantize.autotune_warmup_linear(model, transpose=not (eval))
        if eval and fused_mlp:
            quantize.autotune_warmup_fused(model)
    model.seqlen = 2048
    print('Done.')
